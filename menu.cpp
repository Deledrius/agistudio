...
#ifdef _WIN32
  setCaption("Windows Linux AGI Studio 1.0");
#else
...
static const char *about_text[] = {
  ...
  "http://samhain.cc.huji.ac.il/agi",
    "",
    "Authors:",
    ...
    : QWidget( parent, name )
{
  ...
  QBoxLayout *b1 = new QHBoxLayout(all,20);

  QPushButton *alogo = new QPushButton(this);
  alogo->setPixmap(QPixmap(logo));
  alogo->setFixedSize(110,100);
  b1->addWidget(alogo);

  QBoxLayout *b2 = new QVBoxLayout(b1,2);

  for(int i=0;about_text[i];i++){
    label[i] = new QLabel(about_text[i],this);
    label[i]->setAlignment(AlignHCenter);
    b2->addWidget(label[i]);
  }
...
"License, version 2 or later, as published by the Free\n"
"Software Foundation.  An exception to this license is made\n"
"for linking to a non-GPL library, specifically the Windows\n"
"non-commercial version of Qt.",this);
  all->addWidget(ll);

  QPushButton *ok = new QPushButton(this);
  ok->setText("OK");
  ok->setMaximumSize(80,60);
